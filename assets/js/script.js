...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // CARGAR NOTAS DEL USUARIO DESDE LOCALSTORAGE
    let notasPorDia = cargarNotasDelUsuario();
    ...
    function cargarNotasDelUsuario() {
        const clave = `notas_${usuarioActual.email}`;
        const notasGuardadas = localStorage.getItem(clave);
        return notasGuardadas ? JSON.parse(notasGuardadas) : {};
    }
    
    function guardarNotasDelUsuario() {
        const clave = `notas_${usuarioActual.email}`;
        localStorage.setItem(clave, JSON.stringify(notasPorDia));
    ...
    botonGuardar.addEventListener('click', function() {
        ...
        guardarNotasDelUsuario();
        
        alert('✅ Nota guardada permanentemente');
    ...
    function mostrarNotasDelDia(claveNota) {
        ...
        notasPorDia[claveNota].forEach(function(nota, index) {
            ...
            btnEliminar.onclick = function() {
                if (confirm('¿Eliminar esta nota?')) {
                    notasPorDia[claveNota].splice(index, 1);
                    guardarNotasDelUsuario();
                    generarCalendario(mesActual, anoActual);
                    mostrarNotasDelDia(claveNota);
